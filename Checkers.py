...
class Player2:
    ...
    def can_jump(self, position, myLayer2):
        '''
        Determines if a piece can make a jump move
        '''
        if myLayer2[position[1]][position[0]].color == self.color:
            # Piece is not on the bottom left corner of the board
            if position[0] > 0 and position[1] < 7:
                # Check if we can jump red piece
                if myLayer2[position[1] + 1][position[0] - 1].color == "Red":
                    if position[0] > 1 and position[1] < 6 and myLayer2[position[1] + 2][position[0] - 2] == Layer2.empty:
                        return True
            ...
            if position[0] < 7 and position[1] < 7:
                # Check if we can jump red piece
                if myLayer2[position[1] + 1][position[0] + 1].color == "Red":
                    if position[0] < 6 and position[1] < 6 and myLayer2[position[1] + 2][position[0] + 2] == Layer2.empty:
                        return True
    ...
    def get_valid_moves(self, myLayer2):
        ...
        # Piece is not on the bottom left corner of the board
        if self.pieceLocation[0] > 0 and self.pieceLocation[1] < 7:
            # Store move if left move is valid
            if myLayer2[self.pieceLocation[1] + 1][self.pieceLocation[0] - 1] == Layer2.empty:
                valid_moves[(self.pieceLocation[0] - 1, self.pieceLocation[1] + 1)] = []
            # Check if we can jump red piece
            elif myLayer2[self.pieceLocation[1] + 1][self.pieceLocation[0] - 1].color == "Red":
                if self.pieceLocation[0] > 1 and self.pieceLocation[1] < 6 and myLayer2[self.pieceLocation[1] + 2][self.pieceLocation[0] - 2] == Layer2.empty:
        ...
        # Piece is not on the bottom right corner of the board
        if self.pieceLocation[0] < 7 and self.pieceLocation[1] < 7:
            # Store move if right move is valid
            if myLayer2[self.pieceLocation[1] + 1][self.pieceLocation[0] + 1] == Layer2.empty:
                valid_moves[(self.pieceLocation[0] + 1, self.pieceLocation[1] + 1)] = []
            # Check if we can jump red piece
            elif myLayer2[self.pieceLocation[1] + 1][self.pieceLocation[0] + 1].color == "Red":
                if self.pieceLocation[0] < 6 and self.pieceLocation[1] < 6 and myLayer2[self.pieceLocation[1] + 2][self.pieceLocation[0] + 2] == Layer2.empty:
                    self.get_jumps((self.pieceLocation[0] + 2, self.pieceLocation[1] + 2), myLayer2, valid_moves, [(self.pieceLocation[0] + 1, self.pieceLocation[1] + 1)])
                    jump_move = True

        if myLayer2[self.pieceLocation[1]][self.pieceLocation[0]].rank == True:
                # Piece is not on the top left corner of the board
                if self.pieceLocation[0] > 0 and self.pieceLocation[1] > 0:
                    # Store move if left move is valid
                    if myLayer2[self.pieceLocation[1] - 1][self.pieceLocation[0] - 1] == Layer2.empty:
                        valid_moves[(self.pieceLocation[0] - 1, self.pieceLocation[1] - 1)] = []
                    # Check if we can jump black piece
                    elif myLayer2[self.pieceLocation[1] - 1][self.pieceLocation[0] - 1].color == "Red":
                        if self.pieceLocation[0] > 1 and self.pieceLocation[1] > 1 and myLayer2[self.pieceLocation[1] - 2][self.pieceLocation[0] - 2] == Layer2.empty:
                            self.get_jumps((self.pieceLocation[0] - 2, self.pieceLocation[1] - 2), myLayer2,valid_moves, [(self.pieceLocation[0] - 1, self.pieceLocation[1] - 1)])
                            jump_move = True

                # Piece is not on the top right corner of the board
                if self.pieceLocation[0] < 7 and self.pieceLocation[1] > 0:
                    # Store move if right move is valid
                    if myLayer2[self.pieceLocation[1] - 1][self.pieceLocation[0] + 1] == Layer2.empty:
                        valid_moves[(self.pieceLocation[0] + 1, self.pieceLocation[1] - 1)] = []
                    # Check if we can jump black piece
                    elif myLayer2[self.pieceLocation[1] - 1][self.pieceLocation[0] + 1].color == "Red":
                        if self.pieceLocation[0] < 6 and self.pieceLocation[1] > 1 and myLayer2[self.pieceLocation[1] - 2][self.pieceLocation[0] + 2] == Layer2.empty:
                            self.get_jumps((self.pieceLocation[0] + 2, self.pieceLocation[1] - 2), myLayer2, valid_moves, [(self.pieceLocation[0] + 1, self.pieceLocation[1] - 1)])
                            jump_move = True
    ...
    def get_jumps_king(self, Location, myLayer2, valid_moves, to_delete):
        '''
        Returns valid jumps for a king
        '''
        # Boolean variables that determine if a jump can be made in a certain direction
        invalid_location_bottom_left = Location[0] - 2 < 0 or Location[1] + 2 > 7 or myLayer2[Location[1] + 1][Location[0] - 1].color != "Red"
        ...
        invalid_location_top_right = Location[0] + 2 > 7 or Location[1] - 2 < 0 or myLayer2[Location[1] - 1][Location[0] + 1].color != "Red"
        ...
        invalid_location_top_left = Location[0] - 2 < 0 or Location[1] - 2 < 0 or myLayer2[Location[1] - 1][Location[0] - 1].color != "Red"
        ...
        invalid_location_bottom_right = Location[0] + 2 > 7 or Location[1] + 2 > 7 or myLayer2[Location[1] + 1][Location[0] + 1].color != "Red"
        ...
        def get_jumps_top_left(Location, myLayer2, valid_moves, to_delete):
            ...
            # Keep looking if the left jump is valid
                ...
                return self.get_jumps_king((Location[0] - 2, Location[1] - 2), myLayer2, valid_moves, to_delete)
    ...
    def get_jumps(self, Location, myLayer2, valid_moves, to_delete):
        '''
        Returns a dictionary with all valid jump moves and all
        pieces that would be deleted if that move is made
        '''

        if myLayer2[self.pieceLocation[1]][self.pieceLocation[0]].rank == True:
            self.get_jumps_king(Location, myLayer2, valid_moves, to_delete)
        else:
            def get_jumps_left(Location, myLayer2, valid_moves, to_delete):
                invalid_location_right = Location[0] + 2 > 7 or Location[1] + 2 > 7 or myLayer2[Location[1] + 1][Location[0] + 1].color != "Red"
                go_right = invalid_location_right or myLayer2[Location[1] + 2][Location[0] + 2] != Layer2.empty

                invalid_location = Location[0] - 2 < 0 or Location[1] + 2 > 7 or myLayer2[Location[1] + 1][Location[0] - 1].color != "Red"

                # store move and deleted pieces if there are no more valid jumps
                if invalid_location or myLayer2[Location[1] + 2][Location[0] - 2] != Layer2.empty:
                    if go_right:
                        valid_moves[Location] = to_delete
                # Keep looking if the left jump is valid
                else:
            ...
            def get_jumps_right(Location, myLayer2, valid_moves, to_delete):
                invalid_location_left = Location[0] - 2 < 0 or Location[1] + 2 > 7 or myLayer2[Location[1] + 1][Location[0] - 1].color != "Red"
                go_left = invalid_location_left or myLayer2[Location[1] + 2][Location[0] - 2] != Layer2.empty

                invalid_location = Location[0] + 2 > 7 or Location[1] + 2 > 7 or myLayer2[Location[1] + 1][Location[0] + 1].color != "Red"
            ...
            return get_jumps_left(Location, myLayer2, valid_moves, to_delete), get_jumps_right(Location, myLayer2,valid_moves, to_delete)
...
class Player:
    ...
    def can_jump(self, position, myLayer2):
        ...
        if myLayer2[position[1]][position[0]].color == self.color:
            ...
            if position[0] > 0 and position[1] > 0:
                # Check if we can jump black piece
                if myLayer2[position[1] - 1][position[0] - 1].color == "Black":
                    if position[0] > 1 and position[1] > 1 and myLayer2[position[1] - 2][position[0] - 2] == Layer2.empty:
                        return True
            ...
            if position[0] < 7 and position[1] > 0:
                # Check if we can jump black piece
                if myLayer2[position[1] - 1][position[0] + 1].color == "Black":
                    if position[0] < 6 and position[1] > 1 and myLayer2[position[1] - 2][position[0] + 2] == Layer2.empty:
                        return True
    ...
    def get_valid_moves(self, myLayer2):
        '''
        Returns a dictionary with all valid moves and all pieces that would
        be deleted if the corresponding move is played
        '''
        valid_moves = {}
        jump_move = False
        ...
        if self.pieceLocation[0] > 0 and self.pieceLocation[1] > 0:
            ...
            if myLayer2[self.pieceLocation[1] - 1][self.pieceLocation[0] - 1] == Layer2.empty:
                valid_moves[(self.pieceLocation[0] - 1, self.pieceLocation[1] - 1)] = []
            # Check if we can jump black piece
            elif myLayer2[self.pieceLocation[1] - 1][self.pieceLocation[0] - 1].color == "Black":
                if self.pieceLocation[0] > 1 and self.pieceLocation[1] > 1 and myLayer2[self.pieceLocation[1] - 2][self.pieceLocation[0] - 2] == Layer2.empty:
                    self.get_jumps((self.pieceLocation[0] - 2, self.pieceLocation[1] - 2), myLayer2, valid_moves, [(self.pieceLocation[0] - 1, self.pieceLocation[1] - 1)])
                    jump_move = True
    ...
    def get_jumps_king(self, Location, myLayer2, valid_moves, to_delete):
        ...
        invalid_location_bottom_left = Location[0] - 2 < 0 or Location[1] + 2 > 7 or myLayer2[Location[1] + 1][Location[0] - 1].color != "Black"
        go_bottom_left = invalid_location_bottom_left or myLayer2[Location[1] + 2][Location[0] - 2] != Layer2.empty or (Location[0] - 1, Location[1] + 1) in to_delete

        invalid_location_top_right = Location[0] + 2 > 7 or Location[1] - 2 < 0 or myLayer2[Location[1] - 1][Location[0] + 1].color != "Black"
        go_top_right = invalid_location_top_right or myLayer2[Location[1] - 2][Location[0] + 2] != Layer2.empty or (Location[0] + 1, Location[1] - 1) in to_delete

        invalid_location_top_left = Location[0] - 2 < 0 or Location[1] - 2 < 0 or myLayer2[Location[1] - 1][Location[0] - 1].color != "Black"
        go_top_left = invalid_location_top_left or myLayer2[Location[1] - 2][Location[0] - 2] != Layer2.empty or (Location[0] - 1, Location[1] - 1) in to_delete

        invalid_location_bottom_right = Location[0] + 2 > 7 or Location[1] + 2 > 7 or myLayer2[Location[1] + 1][Location[0] + 1].color != "Black"
        go_bottom_right = invalid_location_bottom_right or myLayer2[Location[1] + 2][Location[0] + 2] != Layer2.empty or (Location[0] + 1, Location[1] + 1) in to_delete
    ...
    def get_jumps(self, Location, myLayer2, valid_moves, to_delete):
        ...
        if myLayer2[self.pieceLocation[1]][self.pieceLocation[0]].rank == True:
            ...
            def get_jumps_left(Location, myLayer2, valid_moves, to_delete):
                to_delete = to_delete[:]
                invalid_location_right = Location[0] + 2 > 7 or Location[1] - 2 < 0 or myLayer2[Location[1] - 1][Location[0] + 1].color != "Black"
                go_right = invalid_location_right or myLayer2[Location[1] - 2][Location[0] + 2] != Layer2.empty

                invalid_location = Location[0] - 2 < 0 or Location[1] - 2 < 0 or myLayer2[Location[1] - 1][Location[0] - 1].color != "Black"
            ...
            def get_jumps_right(Location, myLayer2, valid_moves, to_delete):
                to_delete = to_delete[:]
                invalid_location_left = Location[0] - 2 < 0 or Location[1] - 2 < 0 or myLayer2[Location[1] - 1][Location[0] - 1].color != "Black"
                go_left = invalid_location_left or myLayer2[Location[1] - 2][Location[0] - 2] != Layer2.empty

                invalid_location = Location[0] + 2 > 7 or Location[1] - 2 < 0 or myLayer2[Location[1] - 1][Location[0] + 1].color != "Black"
...
class Layer2:
    ...
    def start_position(self):
        ...
        '''self.squares[2][1] = Piece("Red", False)
        self.squares[0][1] = Piece("Black", False)
        self.squares[0][3] = Piece("Black", False)'''
